...
def load_demo(url_params, request: gr.Request):
    ...
    if args.model_list_mode == "reload":
        models, all_models = get_model_list(controller_url, args.register_api_endpoint_file, False)
...
def _prepare_text_with_image(state, text, image):
    if image is not None:
        ...
        image = state.conv.convert_image_to_base64(image)  # PIL type is not JSON serializable
...
if __name__ == "__main__":
    ...
    models, all_models = get_model_list(args.controller_url, args.register_api_endpoint_file, False)
